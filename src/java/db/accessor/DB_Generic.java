 ...
 * databases. It has been successfully tested on MS-SQL Server 7 and 2000. <p>
 * Use this class when you need to use JDBC version 1 and typical, ANSI-standard
...
public class DB_Generic implements DBAccessor {
     ...
     * should never be closed, but rather returned to the pool. <p> As an
     * alternative to using this method, other member methods is this class
     * offer a boolean switch to close the connection automatically.
     *
    ...
    /**
     * Can be used to perform general purpose sql queries by date range on a
     * single table. No other criteria are supported. In this implementation
     * dates are delimited by the single quote (') character.
     *
     * @param returnFields - an array of Strings containing names of fields to
     * be returned by query
     * @param dateField - a String naming the field in the db representing the
     * start and end date
     * @param startDateValue - a String naming the value in the db representing
     * the start date. This value is not tested for format validity. \
     * @param endDateValue - a String naming the value in the db representing
     * the end date. This value is not tested for format validity.
     * @param isASC - true if sort order ascending by date field; false if
     * descending
     * @param closeConnection - true if connection should be closed
     * automatically; if false, connection must be explicitly closed using the
     * closeConnection method.
     ...
     * be null if the query fails to return any records.
     * @throws SQLException if database access error or illegal sql
     * @throws IllegalArgumentException if any of the date parametersare null or
     * have zero length
     * @throws Exception for all other problems
     */
    @Override
    ...
    /**
     * Inserts a record into a table based on a
     * <code>List</code> of column descriptors and a one-to-one mapping of an
     * associated
     * <code>List</code> of column values.
     *
